"use client";

import { useCallback, useEffect, useRef, useState } from "react";
import Link from "next/link";
import { ArrowLeft, ArrowUpDown, Volume2 } from "lucide-react";
import Backdrop from "@/app/components/Backdrop";
import GameOverModal from "@/app/components/GameOverModal";
import RoundFooter from "@/app/components/RoundFooter";
import { PRESET_ICONS, PRESETS, useAudio } from "../audio";
import {
  buildModeQuiz,
  droneNote,
  EAR_MODES,
  findMode,
  modeNotes,
  modeNotesRoundTrip,
  type ModeQuestion,
} from "@/lib/modeEar";
import { ROUND_LENGTH } from "@/lib/roundLength";

/** Lo que tarda en pasar de una nota de la escala a la siguiente. */
const GAP_MS = 430;

interface Round {
  questions: ModeQuestion[];
  answers: (string | null)[];
}

const createRound = (): Round => ({
  questions: buildModeQuiz(ROUND_LENGTH),
  answers: Array<string | null>(ROUND_LENGTH).fill(null),
});

export default function ModeEarGame() {
  const { playOverDrone } = useAudio();

  const [round, setRound] = useState<Round | null>(null);
  const [step, setStep] = useState(0);
  const [gameOver, setGameOver] = useState(false);
  /** true mientras estás mirando una pregunta vieja en vez de jugando. */
  const [reviewing, setReviewing] = useState(false);
  const [presetIdx, setPresetIdx] = useState(0);
  /** Qué grado de la escala va sonando: -1 = ninguna. */
  const [playIndex, setPlayIndex] = useState(-1);
  /** Si va subiendo o bajando, para que la estela de barras siga a la nota. */
  const [playDir, setPlayDir] = useState<"up" | "down">("up");
  /**
   * Con esto puesto la escala suena de ida y vuelta, aquí y al pasar de
   * pregunta. Cambiarlo no toca nada: solo deja dicho cómo sonará la próxima
   * vez que se escuche.
   */
  const [roundTrip, setRoundTrip] = useState(false);

  const advanceTimerRef = useRef<number | null>(null);
  const flashTimersRef = useRef<number[]>([]);
  const autoPlayRef = useRef(true);

  // La ronda se sortea con Math.random(): armarla en el render del servidor
  // daría una distinta al hidratar.
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setRound(createRound());
    autoPlayRef.current = true;
  }, []);

  const clearFlashTimers = () => {
    flashTimersRef.current.forEach((id) => window.clearTimeout(id));
    flashTimersRef.current = [];
  };

  useEffect(
    () => () => {
      if (advanceTimerRef.current) window.clearTimeout(advanceTimerRef.current);
      clearFlashTimers();
    },
    [],
  );

  const total = round?.questions.length ?? 0;
  const question = round?.questions[step];
  const answered = round?.answers[step] ?? null;
  const correctCount =
    round?.answers.reduce<number>(
      (sum, answer, index) =>
        answer === round.questions[index].modeId ? sum + 1 : sum,
      0,
    ) ?? 0;

  const play = useCallback(
    (target?: ModeQuestion) => {
      if (!target) return;
      clearFlashTimers();

      const notes = roundTrip ? modeNotesRoundTrip(target) : modeNotes(target);
      // Bajando se repiten los mismos grados al revés, así que la nota que
      // suena y la barra que se enciende dejan de ir a la par.
      const bars = findMode(target.modeId).intervals.length;

      notes.forEach((_, index) => {
        const bar = index < bars ? index : 2 * bars - 2 - index;
        flashTimersRef.current.push(
          window.setTimeout(() => {
            setPlayIndex(bar);
            setPlayDir(index < bars ? "up" : "down");
          }, index * GAP_MS),
        );
      });
      flashTimersRef.current.push(
        window.setTimeout(() => setPlayIndex(-1), notes.length * GAP_MS),
      );

      playOverDrone(notes, GAP_MS, presetIdx, droneNote(target));
    },
    [playOverDrone, presetIdx, roundTrip],
  );

  useEffect(() => {
    if (!question || gameOver || !autoPlayRef.current) return;
    autoPlayRef.current = false;
    const timer = window.setTimeout(() => play(question), 150);
    return () => window.clearTimeout(timer);
  }, [question, gameOver, play]);

  /** En qué pregunta va la partida. -1 = ya están todas contestadas. */
  const liveStep = round?.answers.findIndex((answer) => answer === null) ?? 0;

  /**
   * Ir a una pregunta ya contestada para volver a oírla y ver qué pusiste.
   * No se puede saltar hacia delante: como mucho, a la que está en juego.
   */
  const goTo = (index: number) => {
    const last = liveStep === -1 ? total - 1 : liveStep;
    if (index < 0 || index > last || index === step) return;

    if (advanceTimerRef.current) window.clearTimeout(advanceTimerRef.current);
    clearFlashTimers();
    setPlayIndex(-1);
    // Volver a la que está en juego no es revisar: es seguir, y por eso
    // vuelve a sonar sola igual que al llegar a una pregunta nueva.
    const back = index !== last;
    setReviewing(back);
    autoPlayRef.current = !back && liveStep !== -1;
    setStep(index);

    // Ya no quedaba ninguna sin contestar: volver a la última es terminar.
    if (liveStep === -1 && index === total - 1) setGameOver(true);
  };

  const choose = (modeId: string) => {
    if (!round || !question || answered || gameOver) return;

    const answers = [...round.answers];
    answers[step] = modeId;
    setRound({ ...round, answers });

    advanceTimerRef.current = window.setTimeout(() => {
      if (step < total - 1) {
        autoPlayRef.current = true;
        setStep(step + 1);
      } else {
        setGameOver(true);
      }
    }, 1500);
  };

  if (!round || !question) {
    return <div className="min-h-screen bg-slate-950" />;
  }

  const correct = answered ? answered === question.modeId : null;
  const notes = modeNotes(question);

  const optionClass = (modeId: string) => {
    if (!answered) {
      return "border-white/10 bg-white/5 text-white hover:-translate-y-0.5 hover:border-violet-300/50 hover:bg-white/10";
    }
    if (modeId === question.modeId) {
      return "border-emerald-400/60 bg-emerald-400/15 text-emerald-100";
    }
    if (modeId === answered) return "border-rose-400/60 bg-rose-400/15 text-rose-100";
    return "border-white/5 bg-white/[0.02] text-white/25";
  };

  return (
    <div className="relative min-h-screen overflow-x-hidden font-sans text-white">
      <Backdrop />

      {gameOver && <GameOverModal correct={correctCount} total={total} />}

      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[960px] flex-col px-4 pb-5 pt-4 md:px-8 md:pb-7 md:pt-5">
        <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 md:gap-4">
          <Link
            href="/"
            className="inline-flex flex-shrink-0 items-center gap-1.5 rounded-full border border-white/15 bg-slate-950/50 px-2.5 py-2 text-[10px] font-black uppercase tracking-[0.18em] text-white/60 backdrop-blur-sm transition hover:border-violet-300/50 hover:text-white md:px-4"
          >
            <ArrowLeft size={14} />
            <span className="hidden sm:inline">Menú</span>
          </Link>

          <div className="min-w-0 text-center">
            <h1
              className="text-balance text-xl font-black italic uppercase leading-tight tracking-tighter text-white sm:text-2xl md:text-3xl"
              style={{ fontFamily: "Chaney, sans-serif" }}
            >
              ¿Qué <span className="text-violet-300">MODO</span> es?
            </h1>
            <p className="mt-1 truncate text-[9px] font-black uppercase tracking-[0.28em] text-white/35">
              Los siete · con pedal
            </p>
          </div>

          <div className="flex flex-shrink-0 items-center gap-1 rounded-full border border-white/10 bg-slate-950/50 p-1 backdrop-blur-sm">
            {PRESETS.map((preset, index) => (
              <button
                key={preset.label}
                type="button"
                onClick={() => setPresetIdx(index)}
                title={preset.label}
                aria-label={preset.label}
                className={`grid h-7 w-7 place-items-center rounded-full text-xs transition md:h-8 md:w-8 md:text-sm ${
                  presetIdx === index ? "bg-violet-400/25" : "opacity-40 hover:opacity-80"
                }`}
              >
                {PRESET_ICONS[index]}
              </button>
            ))}
          </div>
        </div>

        <main className="mx-auto flex w-full max-w-[760px] flex-1 flex-col justify-start pb-10 pt-6 md:pb-12 md:pt-8">
          <p className="mb-3 text-center text-xs text-white/35">
            Debajo suena la fundamental todo el rato. La escala sube encima: lo
            que cambia de un modo a otro es cómo choca con ella.
          </p>

          <div className="mb-7 flex flex-col items-center gap-4">
            <div className="relative flex min-h-[190px] w-full max-w-[620px] flex-col items-center justify-center gap-5 overflow-hidden rounded-[2.25rem] border-4 border-white bg-white px-5 py-6 shadow-2xl md:min-h-[230px] md:rounded-[3rem]">
              <div className="absolute right-7 top-5 text-2xl font-black italic text-black/10">
                #{step + 1}
              </div>

              {/* Una barra por nota de la escala: se encienden según suenan y
                  su altura dibuja por dónde va subiendo. */}
              <div className="flex items-end justify-center gap-1.5 md:gap-2">
                {notes.map((_, index) => (
                  <span
                    key={index}
                    className={`w-5 rounded-t-md border-2 border-black/70 transition-all duration-150 md:w-7 ${
                      playIndex === index
                        ? "bg-violet-400 shadow-[3px_3px_0px_#000]"
                        : playIndex >= 0 &&
                            (playDir === "up" ? index < playIndex : index > playIndex)
                          ? "bg-stone-400"
                          : "bg-stone-200"
                    }`}
                    style={{ height: `${34 + index * 9}px` }}
                  />
                ))}
              </div>

              <span className="text-[11px] font-black uppercase tracking-[0.22em] text-black/30">
                {answered ? findMode(question.modeId).label : "Escala + pedal"}
              </span>
            </div>

            <div className="flex flex-wrap items-center justify-center gap-3">
              <button
                type="button"
                onClick={() => play(question)}
                className="inline-flex items-center gap-2 rounded-full border border-violet-300/30 bg-violet-400/10 px-5 py-2.5 text-xs font-black uppercase tracking-[0.18em] text-violet-100 transition hover:border-violet-300/60 hover:bg-violet-400/20"
              >
                <Volume2 size={14} strokeWidth={1.8} />
                Escuchar
              </button>

              {/* Ida y vuelta cuesta el doble de tiempo, así que no viene
                  puesto: se enciende y se queda encendido el resto de la
                  ronda. */}
              <button
                type="button"
                onClick={() => setRoundTrip(!roundTrip)}
                aria-pressed={roundTrip}
                className={`inline-flex items-center gap-2 border-2 border-black px-2 py-1 text-[9px] font-black uppercase tracking-[0.14em] text-black shadow-[2px_2px_0px_#000] transition active:translate-x-[2px] active:translate-y-[2px] active:shadow-none ${
                  roundTrip ? "bg-violet-300" : "bg-stone-300 hover:bg-white"
                }`}
              >
                {/* Interruptor de verdad, con su carril y su pastilla: el
                    mismo negro duro y la misma sombra maciza que la tarjeta. */}
                <span
                  className={`flex h-4 w-7 items-center border-2 border-black p-[2px] transition-colors ${
                    roundTrip ? "bg-black" : "bg-white"
                  }`}
                >
                  <span
                    className={`h-full w-[10px] border-2 border-black transition-transform duration-150 ${
                      roundTrip
                        ? "translate-x-[10px] bg-violet-300"
                        : "translate-x-0 bg-stone-400"
                    }`}
                  />
                </span>

                <ArrowUpDown size={11} strokeWidth={3} />
                Up / Down
              </button>
            </div>
          </div>

          <div className="mx-auto grid w-full max-w-[620px] grid-cols-2 gap-3 sm:grid-cols-4">
            {EAR_MODES.map((mode) => (
              <button
                key={mode.id}
                type="button"
                onClick={() => choose(mode.id)}
                disabled={Boolean(answered)}
                className={`min-h-[64px] rounded-2xl border px-3 py-4 text-base font-black tracking-tight backdrop-blur-sm transition duration-150 disabled:cursor-default md:text-lg ${optionClass(mode.id)}`}
              >
                {mode.label}
              </button>
            ))}
          </div>

          <p
            className={`mt-5 text-center text-xs font-black uppercase tracking-[0.2em] transition-opacity ${
              correct === true
                ? "text-emerald-300 opacity-100"
                : correct === false
                  ? "text-rose-300 opacity-100"
                  : "opacity-0"
            }`}
          >
            {correct === true
              ? "¡Bien!"
              : `Dijiste ${answered ? findMode(answered).label : ""}`}
          </p>
        </main>

        <RoundFooter
          step={step}
          total={total}
          liveStep={liveStep}
          results={round.questions.map((item, index) => {
            const given = round.answers[index];
            return given === null ? null : given === item.modeId;
          })}
          correctCount={correctCount}
          reviewing={reviewing}
          onGoTo={goTo}
        />
      </div>
    </div>
  );
}
