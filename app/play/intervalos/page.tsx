...
export default function IntervalosGame() {
  ...
  const botonesIntervalos = [
    "b2", "2", "b3", "3", "4", "#4/b5", "5", "b6", "6", "b7", "7", "8"
  ];
  ...
  // Manejo de carga de imagen con detección de error
  useEffect(() => {
    setIsImageLoading(true);
    const img = new Image();
    // Usamos encodeURI para manejar los espacios en nombres de carpetas como "1. b2"
    const safePath = encodeURI(currentQuestion.image);
    img.src = safePath;
    
    img.onload = () => setIsImageLoading(false);
    img.onerror = () => {
      console.error("No se pudo cargar la imagen en:", safePath);
      setIsImageLoading(false); // Quitamos el spinner aunque falle para ver el broken link
    };
  ...
  const handleAnswer = (intervaloSeleccionado: string) => {
    if (userAnswers[step] !== null || gameOver) return;
    setIsReviewing(false);

    const isCorrect = intervaloSeleccionado === currentQuestion.answer;
    const newResults = [...results];
    newResults[step] = isCorrect ? "correct" : "wrong";
    setResults(newResults);

    const newAnswers = [...userAnswers];
    newAnswers[step] = intervaloSeleccionado;
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-slate-900 bg-cover bg-center overflow-x-hidden"
      ...
      <div className="absolute top-24 left-0 right-0 flex justify-between px-32 pointer-events-none z-0">
        <img src="/assets/logo21stCM_no_white_1.png" className="h-28 w-auto drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] opacity-90" alt="logo" />
        <img src="/assets/logo21stCM_no_white_1.png" className="h-28 w-auto drop-shadow-[0_0_15px_rgba(255,255,255,0.3)] opacity-90" alt="logo" />
      </div>
      ...
      <div className="min-h-screen bg-black/10 flex flex-col items-center justify-center p-6 z-10">
        ...
        <div className="relative flex flex-col items-center w-full max-w-md mb-8">
          <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl w-full h-48 flex items-center justify-center border-4 border-white relative overflow-hidden">
            ...
            <img 
              ...
              alt="Intervalo" 
              className={`max-h-full w-auto object-contain transition-all duration-300 ${isImageLoading ? 'opacity-0' : 'opacity-100'}`} 
            />
          </div>
          ...
          <div className={`absolute -bottom-12 left-0 right-0 z-30 transition-all duration-500 transform ${isReviewing && userAnswers[step] !== null ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-4 opacity-0 scale-95 pointer-events-none'}`}>
            <div className="mx-auto w-48 h-16 rounded-2xl border-2 border-fuchsia-400/50 bg-black/80 backdrop-blur-xl flex flex-col items-center justify-center shadow-2xl">
              <span className="text-[8px] text-fuchsia-400 uppercase font-black tracking-widest">Solución</span>
              <span className="text-xl font-black text-white italic lowercase tracking-tighter" style={{ fontFamily: 'Chaney, sans-serif' }}>
        ...
        <div className={`bg-black/40 p-8 rounded-[3rem] border border-white/10 w-full max-w-6xl backdrop-blur-md transition-all ${userAnswers[step] !== null ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
            {botonesIntervalos.map(int => (
              <button 
                key={int} 
                disabled={userAnswers[step] !== null || gameOver} 
                onClick={() => handleAnswer(int)} 
                className="py-5 rounded-xl border border-white/10 bg-white/5 text-white hover:bg-fuchsia-500 hover:text-white transition-all active:scale-95 shadow-sm"
              >
                <span className="text-sm font-semibold tracking-normal font-sans">{int}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="w-full max-w-4xl mt-16 flex items-center justify-center gap-4 px-4">
          <button onClick={goBack} className={`shrink-0 px-6 py-3 bg-white/5 border border-white/10 text-white text-[10px] font-bold rounded-full uppercase tracking-widest hover:bg-white/10 transition-all ${step === 0 ? 'opacity-0' : 'opacity-100'}`}>
            ← Anterior
          </button>
            ...
            {results.map((res, i) => (
              <div 
                ...
                className={`w-6 h-6 rounded-md border flex items-center justify-center text-[8px] font-black cursor-pointer transition-all ${
                  ...
                  i === step ? "border-fuchsia-400 bg-white/20 text-white scale-110 shadow-[0_0_15px_rgba(192,38,211,0.5)]" : "border-white/5 text-white/10"
                }`}
              >
                {i + 1}
              </div>
          ...
          <button onClick={goNext} className={`shrink-0 px-8 py-3 bg-fuchsia-600 text-white text-[10px] font-black rounded-full uppercase tracking-widest hover:scale-105 transition-all shadow-xl ${ isReviewing ? 'opacity-100' : 'opacity-0 pointer-events-none' }`}>
            Siguiente →
          </button>
        </div>
      ...
      {gameOver && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md">
          <div className="text-center p-12 bg-white/5 rounded-[4rem] border border-white/10 max-w-sm w-full mx-4 shadow-2xl">
            ...
            <div className="text-5xl font-black text-fuchsia-400 my-8 italic" style={{ fontFamily: 'Chaney, sans-serif' }}>
              {results.filter(r => r === "correct").length}<span className="text-white/20 text-2xl mx-2">/</span>24
            </div>
            <button onClick={() => window.location.reload()} className="w-full py-4 bg-fuchsia-600 text-white font-black rounded-2xl text-xs uppercase tracking-widest hover:scale-105 transition-all">Reiniciar</button>
          </div>
        </div>
      )}
    </div>
