...
export default function ArmadurasGame({ claves }: { claves: Clave[] }) {
  ...
  /**
   * La ronda se arma al montar y con las claves que trae la ruta.
   *
   * Va en un efecto y no en el estado inicial porque se sortea con
   * `Math.random()`: hacerlo durante el render del servidor daría una ronda
   * distinta al hidratar.
   */
  useEffect(() => {
    const shuffled = barajar(getArmaduras(claves)).slice(0, ROUND_LENGTH);
