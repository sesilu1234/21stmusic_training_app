...
export default function ArmadurasGame() {
  ...
  // Solo las notas, sin M o men
  const todasLasNotas = useMemo(() => [
    "Do", "Do#", "Reb", "Re", "Mib", "Mi", "Fa", "Fa#", "Solb", "Sol", "Lab", "La", "Sib", "Si", "Dob"
  ], []);
  ...
  const handleNoteClick = (notaSeleccionada: string) => {
    if (gameOver || userAnswers[step] !== null) return;
    setIsNavigatingBack(false);

    // Reconstruimos la opción lógica para comparar (ej: "La" + " men" o "La" + " M")
    const opcionLogica = esPreguntaMayor ? `${notaSeleccionada} M` : `${notaSeleccionada} men`;
    
    const isCorrect = opcionLogica === respuestaCorrecta;
    const newResults = [...results];
    newResults[step] = isCorrect ? "correct" : "wrong";
    setResults(newResults);

    const newAnswers = [...userAnswers];
    newAnswers[step] = notaSeleccionada; // Guardamos solo la nota
  ...
  const showSolutionBox = isNavigatingBack && userAnswers[step] !== null;
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-6 bg-cover bg-center overflow-hidden"
      ...
      <button onClick={() => router.push("/play")} className="absolute top-8 left-8 text-white/50 hover:text-white text-[10px] font-bold uppercase tracking-widest bg-black/20 px-4 py-2 rounded-full border border-white/10 z-20 transition-all">
        ← Menú Principal
      </button>
      ...
      <div className="relative flex flex-col items-center w-full max-w-md mb-8">
        ...
        <div className={`absolute -bottom-12 left-0 right-0 z-30 transition-all duration-500 transform ${showSolutionBox ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-4 opacity-0 scale-95 pointer-events-none'}`}>
      ...
      <div className={`bg-black/40 p-8 rounded-[3rem] border border-white/10 w-full max-w-6xl backdrop-blur-md transition-all ${userAnswers[step] !== null ? 'opacity-20 pointer-events-none' : 'opacity-100'}`}>
        <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-8 gap-4 lg:grid-cols-15">
      ...
      <div className="w-full max-w-2xl flex flex-col items-center gap-6 mt-12">
        <div className="flex gap-4 h-10 items-center">
          {step > 0 && (
            <button onClick={goBack} className="px-6 py-2 bg-white/5 border border-white/10 text-white text-[10px] font-bold rounded-full uppercase tracking-widest hover:bg-white/10 transition-all">
              ← Anterior
            </button>
          )}
          {(userAnswers[step] !== null && step < 23) && (
            <button onClick={goNext} className="px-6 py-2 bg-amber-500 text-black text-[10px] font-bold rounded-full uppercase tracking-widest hover:scale-105 transition-all">
              Siguiente →
            </button>
          )}
        </div>
