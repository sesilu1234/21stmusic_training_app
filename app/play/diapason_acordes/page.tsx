...
export default function ChordsGame() {
  ...
  const getSolucionTexto = () => {
    const mapping: any = { "Mayor": "Mayores", "Menor": "Menores", "Aumentado": "aug", "Disminuido": "dim" };
    return Object.keys(mapping).find(k => mapping[k] === currentQuestion.answer);
  };
      ...
      <div className="min-h-screen bg-black/10 flex flex-col items-center justify-center p-6 z-10">
        ...
        <div className="mb-6 text-center mt-20">
          <h2 className="text-white text-3xl font-black italic tracking-tighter leading-tight" 
            ...
            ¿<span className="uppercase">Q</span>ué tipo de <span className="text-black mx-2 drop-shadow-[0_1.2px_1.2px_rgba(255,255,255,0.8)] uppercase">ACORDE</span> es este?
          </h2>
        </div>
        ...
        <div className="relative flex flex-col items-center w-full max-w-md mb-8">
          ...
          <div className={`absolute -bottom-12 left-0 right-0 z-30 transition-all duration-200 transform ${ (isReviewing || showFeedback === "wrong") && userAnswers[step] !== null ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-4 opacity-0 pointer-events-none'}`}>
            <div className="mx-auto w-48 h-14 rounded-2xl border-2 border-amber-400/50 bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center shadow-2xl">
              <span className="text-[7px] text-amber-400 uppercase font-black tracking-widest mb-0.5">Solución</span>
              <span className="text-sm font-bold text-white font-sans">{getSolucionTexto()}</span>
            </div>
          </div>
        </div>
